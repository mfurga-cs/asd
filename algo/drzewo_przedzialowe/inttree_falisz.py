...
def tree_maxi(X, I):
  ...
  if a < X.cut:
    left = tree_maxi(X.lchild, I)
  if b > X.cut:
    right = tree_maxi(X.rchild, I)

  return max(left, right)

def tree_set(X, I, value):
  (a, b) = I
  if a <= X.left and b >= X.right:
    X.max = value
    return
  ...
  if a < X.cut:
    tree_set(X.lchild, I, value)
  if b > X.cut:
    tree_set(X.rchild, I, value)

  X.max = max(X.lchild.max, X.rchild.max)
