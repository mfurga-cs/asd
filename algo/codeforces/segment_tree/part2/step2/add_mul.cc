/*
  ...
  Operacje:
    1. update(a, b, v) - mnoży elementy z przedziału a do b przez v.
...
struct SegmentTree {
  ...
  int64_t sum_opt(int64_t a, int64_t b) {
    return (a + b) % (int64_t)(1e9 + 7);
  }

  int64_t mul_opt(int64_t a, int64_t b) {
    return (a * b) % (int64_t)(1e9 + 7);
  }
  ...
  void update(int a, int b, int64_t value, int vertex, int left, int right) {
    ...
    if (a <= left && b >= right) {
      sum[vertex] = mul_opt(sum[vertex], value);
      mul[vertex] = mul_opt(mul[vertex], value);
      return;
    }
    ...
    sum[vertex] = mul_opt(sum_opt(sum[LEFT(vertex)], sum[RIGHT(vertex)]), mul[vertex]);
  }
  ...
  int64_t query(int a, int b, int vertex, int left, int right) {
    ...
    return mul_opt(sum_opt(l, r), mul[vertex]);
  }
